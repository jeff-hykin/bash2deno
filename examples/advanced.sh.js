...
// Busybox polyfills
if ( /* FIXME: cp --help 2>&1 | grep -q -- --no-clobber */) {
...
async function get_avail_fd(...args) { const { local, env } = makeScope({ args })
    ...
    local.x = ""
    /* FIXME: for x in $(seq 1 $(ulimit -n));  */{

        if ( /* FIXME: -a "/proc/$BASHPID/fd/$x" */) {
            console.log(`${env.x}`)
...
async function init_lock(...args) { const { local, env } = makeScope({ args })
    ...
    await $`umask 0555`
    /* FIXME: eval "exec $LOCK_FD>$LOCK_FILE" > /dev/null 2>&1 || return 1 */
    await $`umask ${env.SCRIPT_UMASK}`
...
async function is_wifi_interface(...args) { const { local, env } = makeScope({ args })
    ...
    /* FIXME: which iw > /dev/null 2>&1 && iw dev $1 info > /dev/null 2>&1 && return 0 */
...
async function get_phy_device(...args) { const { local, env } = makeScope({ args })
    ...
    local.x = ""
    /* FIXME: for x in /sys/class/ieee80211/*;  */{
...
async function can_be_sta_and_ap(...args) { const { local, env } = makeScope({ args })
    ...
    /* FIXME: get_adapter_info "$1" | grep -E '{.* managed.* AP.*}' > /dev/null 2>&1 && return 0 */
    /* FIXME: get_adapter_info "$1" | grep -E '{.* AP.* managed.*}' > /dev/null 2>&1 && return 0 */
    return exitCodeOfLastChildProcess = 1
...
async function can_be_ap(...args) { const { local, env } = makeScope({ args })
    ...
    /* FIXME: get_adapter_info "$1" | grep -E '\* AP$' > /dev/null 2>&1 && return 0 */
    return exitCodeOfLastChildProcess = 1
...
async function is_wifi_connected(...args) { const { local, env } = makeScope({ args })
    ...
    if ( env.USE_IWCONFIG == 0) {
        /* FIXME: iw dev "$1" link 2>&1 | grep -E '^Connected to' > /dev/null 2>&1 && return 0 */
    
    } else {

        /* FIXME: iwconfig "$1" 2>&1 | grep -E 'Access Point: [0-9a-fA-F]{2}:' > /dev/null 2>&1 && return 0 */
...
async function alloc_new_iface(...args) { const { local, env } = makeScope({ args })
    ...
    while (true) {
        ...
        if ( /* FIXME: ! is_interface $prefix$i && [[ ! -f $COMMON_CONFDIR/ifaces/$prefix$i ]] */) {
...
async function get_new_macaddr(...args) { const { local, env } = makeScope({ args })
    ...
    for (env.i = 1; env.i <= 255; env.i++) {
        ...
        /* FIXME: (get_all_macaddrs | grep "$NEWMAC" > /dev/null 2>&1) || break */
...
async function haveged_watchdog(...args) { const { local, env } = makeScope({ args })
    ...
    while (true) {
        ...
        if ( await $`cat '/proc/sys/kernel/random/entropy_avail'`.text() < 1000) {
            if ( await $E: ! which haveged *`.stdout(...$stdout)) {
                if ( env.show_warn == 1) {
                    console.log(`WARN: Low entropy detected. We recommend you to install \`haveged'`)
                    env.show_warn = 0
                
                }
            } else if ( await $E: ! pidof haveged *`.stdout(...$stdout)) {
...
async function networkmanager_exists(...args) { const { local, env } = makeScope({ args })
    ...
    local.NM_VER = ""
    /* FIXME: which nmcli > /dev/null 2>&1 || return 1 */
...
async function networkmanager_is_running(...args) { const { local, env } = makeScope({ args })
    ...
    if ( env.NM_OLDER_VERSION == 1) {
        env.NMCLI_OUT = /* FIXME: nmcli -t -f RUNNING nm 2>&1 | grep -E '^running$' */.text()
    
    } else {

        env.NMCLI_OUT = /* FIXME: nmcli -t -f RUNNING g 2>&1 | grep -E '^running$' */.text()
...
async function networkmanager_knows_iface(...args) { const { local, env } = makeScope({ args })
    ...
    /* FIXME: nmcli -t -f DEVICE d 2>&1 | grep -Fxq "$1" */
...
async function networkmanager_iface_is_unmanaged(...args) { const { local, env } = makeScope({ args })
    ...
    /* FIXME: (nmcli -t -f DEVICE,STATE d 2>&1 | grep -E "^$1:unmanaged$" > /dev/null 2>&1) || return 1 */
...
async function networkmanager_add_unmanaged(...args) { const { local, env } = makeScope({ args })
    ...
    /* FIXME: for x in $UNMANAGED;  */{

        if ( if (  /* FIXME: !  [[ $x == "mac:${MAC}" ]] */) {
    ...
    if ( await $E: ! grep -E '^\[keyfile\]' ${NETWORKMANAGER_CONF} *`.stdout(...$stdout)) {
        await $`echo -e '
...
async function _cleanup(...args) { const { local, env } = makeScope({ args })
    ...
    local.found = 0
    /* FIXME: for x in $(list_running_conf);  */{
    ...
    if ( /* FIXME: ! has_running_instance */) {
        // kill common processes
        /* FIXME: for x in $COMMON_CONFDIR/*.pid;  */{
        ...
        // set old ip_forward
        if ( /* FIXME: -f $COMMON_CONFDIR/ip_forward */) {
        ...
        if ( /* FIXME: -f $COMMON_CONFDIR/bridge-nf-call-iptables */) {
    ...
    if ( env.SHARE_METHOD !== `none`) {
        if ( env.SHARE_METHOD === `nat`) {
            ...
            if ( /* FIXME: ! is_bridge_interface $INTERNET_IFACE */) {
...
async function list_running_conf(...args) { const { local, env } = makeScope({ args })
    ...
    local.x = ""
    await $`mutex_lock`
    /* FIXME: for x in /tmp/create_ap.*;  */{

        if ( /* FIXME: -f $x/pid && -f $x/wifi_iface && -d /proc/$(cat $x/pid) */) {
            console.log(`${env.x}`)
...
async function print_client(...args) { const { local, env } = makeScope({ args })
    ...
    if ( /* FIXME: -f $CONFDIR/dnsmasq.leases */) {
...
async function send_stop(...args) { const { local, env } = makeScope({ args })
    ...
    /* FIXME: for x in $(list_running | grep -E " \(?${1}( |\)?\$)" | cut -f1 -d' ');  */{
...
async function write_config(...args) { const { local, env } = makeScope({ args })
    ...
    if ( env.STORE_CONFIG !== `/etc/create_ap.conf`) {
        if ( fs.existsSync(`'${env.STORE_CONFIG}'`)) {
            if ( /* FIXME: ! pkexec --user "$(id -nu $PKEXEC_UID)" test -w "$STORE_CONFIG" */) {
        ...
        } else if ( /* FIXME: ! pkexec --user "$(id -nu $PKEXEC_UID)" test -w "$(dirname "$STORE_CONFIG")" */) {
    ...
    } else if ( await $E: ! eval 'echo -n > "$STORE_CONFIG"' *`.stdout(...$stdout)) {
    ...
    await $E: for config_opt in "${CONFIG_OPTS[@]}";  */{
...
if ( env.LOAD_CONFIG.length > 0) {
    ...
    /* FIXME: for x in WIFI_IFACE INTERNET_IFACE SSID PASSPHRASE;  */{
...
if ( env.DAEMONIZE == 1) {
    ...
    /* FIXME: RUNNING_AS_DAEMON=1 setsid "$0" "${ARGS[@]}" >>$DAEMON_LOGFILE 2>&1 */ /* FIXME: & */
    await $`exit 0`
...
if ( env.NEW_MACADDR.length > 0) {
    if ( /* FIXME: ! is_macaddr "$NEW_MACADDR" */) {
        await $`echo 'ERROR: '"'"'${env.NEW_MACADDR}'"'"' is not a valid MAC address' >&2`
        await $`exit 1`
    
    }

    if ( /* FIXME: ! is_unicast_macaddr "$NEW_MACADDR" */) {
    ...
    if ( await $`tty -s`) {
        ...
        while (true) {
            ...
            if ( env.USE_PSK == 0) {
                ...
                console.log(``)
                if ( if (  /* FIXME: !  [[ ${#PASSPHRASE} -gt 0 && ${#PASSPHRASE} -lt 8 ]] */) {
...
if ( if (  env.SHARE_METHOD !== `none`) {
                                     /* FIXME: ! is_interface $INTERNET_IFACE */
...
if ( env.NO_VIRT == 0) {
    ...
    if ( /* FIXME: networkmanager_is_running && [[ $NM_OLDER_VERSION -eq 0 ]] */) {
    ...
    if ( /* FIXME: is_wifi_connected ${WIFI_IFACE} && [[ $FREQ_BAND_SET -eq 0 ]] */) {
        ...
        if ( env.WIFI_IFACE_CHANNEL != env.CHANNEL) {
            if ( /* FIXME: ( get_adapter_info ${IFACE} | grep "#channels <= 2" -q ) */
            ) {
    ...
    } else if ( /* FIXME: is_wifi_connected ${WIFI_IFACE} && [[ $FREQ_BAND_SET -eq 1 ]] */) {
...
if ( /* FIXME: networkmanager_exists && ! networkmanager_iface_is_unmanaged ${WIFI_IFACE} */) {
...
if ( env.SHARE_METHOD !== `none`) {
    ...
    if ( env.SHARE_METHOD === `nat`) {
        ...
        /* FIXME: echo 1 > /proc/sys/net/ipv4/conf/$INTERNET_IFACE/forwarding || die */
        /* FIXME: echo 1 > /proc/sys/net/ipv4/ip_forward || die */
...
await $`${env.STDBUF_PATH} ${env.HOSTAPD} ${env.HOSTAPD_DEBUG_ARGS} ${env.CONFDIR}'/hostapd.conf'` /* FIXME: & */
env.HOSTAPD_PID = env["!"]
await $`echo ${env.HOSTAPD_PID} > $CONFDIR/hostapd.pid`

if ( /* FIXME: ! wait $HOSTAPD_PID */) {
    await $`echo -e '
